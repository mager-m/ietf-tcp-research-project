...
int StatedataManager::query_statedate_from_machine(clicon_handle h, cvec *nsc, char *xpath, cxobj *xstate) {
    ...
    try {
        ...
        auto *tcpStats = TCPStatisticsManager::readStatistics();
        auto connectionList = TCPConnectionManager::readConnections();
        ...
        cprintf(cbuf, "<currently-established>%s</currently-established>", tcpStats->currentlyEstablished.c_str());
